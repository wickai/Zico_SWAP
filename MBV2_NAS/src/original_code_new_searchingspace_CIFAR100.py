...
def get_cifar100_dataloaders(root, batch_size, num_workers=2,
                            use_cutout=False, cutout_length=16):
    """
    构造CIFAR-100 DataLoader，支持cutout
    最终训练用
    """
    transform_list = [
        transforms.RandomCrop(32, padding=4),
    ...
    train_ds = datasets.CIFAR100(root, train=True, download=True, transform=transform_train)
    test_ds  = datasets.CIFAR100(root, train=False, download=True, transform=transform_test)
...
def parse_args():
    ...
    parser.add_argument("--data_path", default="./data", type=str, help="CIFAR-100 dataset path")
    ...
    parser.add_argument("--small_input", action="store_true", default=True, help="适配CIFAR-100的小输入")
    parser.add_argument("--num_classes", default=100, type=int, help="类别数")
...
def main():
    ...
    train_ds = datasets.CIFAR100(root=args.data_path, train=True, download=True, transform=transform_eval)
